    ...
    with client() as c:
        for data_type in ("raw", "1m", "1h", "1d"):
            ...
            if not check(resp.status_code == 200, f"{data_type}: current -> 200"):
                continue
            body = resp.json()
            check("timestamp" in body, f"{data_type}: response has 'timestamp'")
            if data_type == "raw":
                check("thi" in body, "raw: response includes computed 'thi'")
                check("temp_c" in body, "raw: response includes computed 'temp_c'")
            else:
                check("sample_count" in body, f"{data_type}: response has 'sample_count'")
                check("tempf_avg" in body, f"{data_type}: response has aggregate columns (tempf_avg)")
            print(f"{body=}")
